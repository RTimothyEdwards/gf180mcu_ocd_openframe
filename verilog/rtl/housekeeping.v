...
// December 2025
//
// PicoRV32 and Picosoc were written by Claire Xenia Wolf
//----------------------------------------------------------
...
//
// This module has been adapted for the GF180MCU openframe
// architecture.
//
//-----------------------------------------------------------
...
// Register 3:  Product ID (= 23) (readonly)
...
// Register 15:  Memory (SRAM) force on (test)
//------------------------------------------------------------
...
`endif
    ...
    mon_div, aux_div, irq, mem_force_ena, reset,
...
`endif
    ...
    (iaddr == 8'h13) ? mon_div :
    (iaddr == 8'h14) ? aux_div :
    (iaddr == 8'h15) ? {7'b0000000, mem_force_ena} :
               8'h00;	// Default
    ...
    if (RSTB == 1'b0) begin
        ...
        dll_div <= 8'd8;	// Default feedback divider divide-by-8
	...
	mem_force_ena <= 1'b0;	// No memory force enable on by default
        irq <= 1'b0;
        reset_reg <= 1'b0;
